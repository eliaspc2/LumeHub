...
async function withLiveRuntime(run) {
  const sandboxPath = await createLiveSandboxPath('lume-hub-wave66-');
  const httpPort = await reservePort();
  const baseUrl = `http://127.0.0.1:${httpPort}`;
  const fetchMock = createLiveFetchMock();
  const groupJid = '120363406000000066@g.us';
  const socketCoordinator = new FakeSocketCoordinator({
    groupJid,
    groupLabel: 'Wave 66 Grupo Home',
    privateChatJid: '351920000066@s.whatsapp.net',
    privateChatLabel: 'Validator Wave66',
  });
  ...
  await writeJson(runtimeConfig.groupSeedFilePath, {
    ...
    groups: [
      {
        groupJid,
        preferredSubject: 'Wave 66 Grupo Home',
        aliases: ['Wave66'],
        courseId: 'wave66-course',
        groupOwners: [
          {
  ...
  try {
    ...
    socketCoordinator.latestSocket.publishQr('wave66-live-qr');
