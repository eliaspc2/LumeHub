...
async function withLiveRuntime(run) {
  const sandboxPath = await createLiveSandboxPath('lume-hub-wave64-');
  ...
  const socketCoordinator = new FakeSocketCoordinator({
    groupJid,
    groupLabel: 'Wave 64 Grupo Shell',
    privateChatJid: '351920000060@s.whatsapp.net',
    privateChatLabel: 'Validator Wave64',
  });
  ...
  await writeJson(runtimeConfig.groupSeedFilePath, {
    ...
    groups: [
      {
        groupJid,
        preferredSubject: 'Wave 64 Grupo Shell',
        aliases: ['Wave64'],
        courseId: 'wave64-course',
        groupOwners: [
          {
  ...
  await writeJson(reserveAFilePath, {
    auth_mode: 'chatgpt',
    tokens: {
      access_token: 'wave64-reserve-a-token',
      account_id: 'wave64-reserve-a',
    },
  });
  await writeJson(reserveBFilePath, {
    auth_mode: 'chatgpt',
    tokens: {
      access_token: 'wave64-reserve-b-token',
      account_id: 'wave64-reserve-b',
    },
  });
  ...
  try {
    ...
    socketCoordinator.latestSocket.publishQr('wave64-live-qr');
...
await withLiveRuntime(async ({ baseUrl, groupJid, fetchMock, runtimeConfig }) => {
  ...
  await assertHeadlessRoute(`${baseUrl}/week?mode=live`, ['Calendario semanal', 'Wave 64 Grupo Shell']);
  ...
  assert.match(await readFile(runtimeConfig.canonicalCodexAuthFile, 'utf8'), /wave64-reserve-b-token/u);
  ...
  const groupChat = await requestJson(baseUrl, '/api/llm/chat', {
    ...
    body: {
      ...
      domainFacts: ['Grupo ativo: Wave 64 Grupo Shell.'],
      memoryScope: {
        scope: 'group',
        groupJid,
        groupLabel: 'Wave 64 Grupo Shell',
  ...
  assert.equal(groupLog?.memoryScope?.groupLabel, 'Wave 64 Grupo Shell');
...
assert.match(gapAuditDoc, /Limpeza final de classes, helpers e variacoes supersedidas/u);
...
assert.match(sourceReadme, /`validate:wave64`/u);
...
assert.match(sourceReadme, /a `Wave 64` migra a shell restante/u);

assert.equal(
  packageJson.scripts['validate:wave64'],
  'corepack pnpm run typecheck && corepack pnpm run build && node --test ./tests/unit/group-repository-owner-override.test.mjs ./tests/unit/codex-auth-router-enabled.test.mjs ./tests/unit/codex-auth-router-multi-account.test.mjs ./tests/integration/wave56-group-mode-routing.test.mjs ./tests/integration/wave57-group-ownership-policy.test.mjs && node --experimental-global-webcrypto ./scripts/validate-wave64.mjs',
);
assert.equal(
...
console.log('validate-wave64: ok');
