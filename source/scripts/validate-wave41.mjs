...
const sandboxPath = await mkdtemp(join(tmpdir(), 'lume-hub-wave41-'));
...
try {
  ...
  const fakeExecutor = {
    async run(input) {
      ...
      await writeFile(
        absoluteTargetPath,
        `${currentContent.trimEnd()}\n\n<!-- workspace-agent-wave41: touched -->\n`,
      );
    ...
    modules: {
      ...
      audienceRouting: {
        ...
        async upsertSenderAudienceRule() {
          throw new Error('Not used in wave41 validation.');
        },
  ...
  assert.equal(completedApply.requestedBy, 'wave41-validator');
  ...
  console.log('validate-wave41: ok');
} finally {
  if (server) {
