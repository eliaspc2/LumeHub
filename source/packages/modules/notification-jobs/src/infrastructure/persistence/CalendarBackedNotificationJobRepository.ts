    ...
    return events
      .flatMap((event) =>
        event.notifications.map((notification) => ({
          ...
          ruleLabel: notification.ruleLabel ?? null,
          mediaAssetId: notification.mediaAssetId ?? null,
          messageTemplate: notification.messageTemplate ?? null,
          llmPromptTemplate: notification.llmPromptTemplate ?? null,
          sendAt: notification.sendAt,
          status: notification.status,
          preparedAt: notification.preparedAt ?? null,
          preparedInstructionId: notification.preparedInstructionId ?? null,
          preparedActionId: notification.preparedActionId ?? null,
    ...
    const updatedEvent = await this.scheduleEventService.updateEvent(
      eventId,
      {
        notifications: jobs.map((job) => ({
          ...
          ruleLabel: job.ruleLabel,
          messageTemplate: job.messageTemplate,
          llmPromptTemplate: job.llmPromptTemplate,
          sendAt: job.sendAt,
          status: job.status,
          preparedAt: job.preparedAt,
          preparedInstructionId: job.preparedInstructionId,
          preparedActionId: job.preparedActionId,
    ...
    const updatedNotifications = event.notifications.map((notification) =>
      notification.jobId === jobId
        ? {
            ...
            ruleLabel: updatedJob.ruleLabel,
            mediaAssetId: updatedJob.mediaAssetId,
            messageTemplate: updatedJob.messageTemplate,
            llmPromptTemplate: updatedJob.llmPromptTemplate,
            sendAt: updatedJob.sendAt,
            status: updatedJob.status,
            preparedAt: updatedJob.preparedAt,
            preparedInstructionId: updatedJob.preparedInstructionId,
            preparedActionId: updatedJob.preparedActionId,
